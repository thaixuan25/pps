...
#chọn ra num điểm gần x0 nhất, tối ưu cho Newton lùi (x0 gần điểm cuối)
def pickPoint(x0, num, x, y):
    if num > len(x):
        print("Lỗi: Số lượng mốc nội suy yêu cầu lớn hơn số lượng mốc trong dữ liệu.")
        sys.exit()

    # Tìm chỉ số `i` của mốc nhỏ nhất mà lớn hơn hoặc bằng x0.
    # np.searchsorted tìm vị trí chèn, ta lấy vị trí bên trái.
    i = np.searchsorted(x, x0, side='left')
    
    # Đảm bảo chỉ số không vượt quá biên nếu x0 lớn hơn tất cả các mốc x.
    i = min(len(x) - 1, i)

    # Xác định chỉ số kết thúc (`end_index`) của dãy mốc sẽ được chọn.
    # Ta muốn cửa sổ `num` điểm kết thúc tại `i`, nhưng phải đảm bảo không vượt
    # ra ngoài biên trái của mảng. Nếu có, ta dịch cửa sổ sang phải.
    end_index = min(len(x), max(num, i + 1))
...
def newton_backward_interpolation(x1, y1):
    ...
    # 1. Tính và lưu trữ bảng sai phân (giống nhau cho tiến và lùi)
    ...
    # Lấy các sai phân lùi cần thiết từ dòng cuối của bảng (đường chéo ngược)
    backward_diffs = np.zeros(n + 1)
    for k in range(n + 1):
        backward_diffs[k] = diff_table[n-k][k]

    # 2. Tính các hệ số b_k = ∇^k(y_n) / k!
...
#vẽ hình
def plot(x, y, x_all, y_all, x0, y0, P_t, h, x_end):
    plt.clf()
...
def in_bang_sai_phan(x1, diff_table):
    n = len(x1)
    print("\n" + "="*90)
    print("BẢNG SAI PHÂN LÙI (BACKWARD DIFFERENCE TABLE)".center(90))
    print("="*90)
    ...
    # Các hệ số cần dùng (y_n, ∇y_n, ∇²y_n,...) nằm ở đường chéo cuối.
    for i in range(n):
...
def calculate_poly_t(newton_coeffs):
    ...
    for k in range(n - 1, -1, -1):
        # P_k(t) = b_k + (t+k)/ (k+1) * P_{k+1}(t)
        # Sửa lại công thức Horner cho Newton lùi
        # P(t) = b0 + b1*t + b2*t(t+1) + ...
        # P(t) = b0 + t*(b1 + (t+1)*(b2 + ...))
        # P_k(t) = b_k + (t+k) * P_{k+1}(t)  <-- đây không phải Horner, đây là Newton
...
def get_polynomial_in_x_str(P_t, h, x_end):
    from numpy.polynomial import Polynomial
    
    # t = (x - x_end) / h = (1/h)x - (x_end/h)
    t_from_x = Polynomial([-x_end / h, 1 / h])
    P_x = P_t(t_from_x)
    poly_x_coeffs = P_x.coef

    # Định dạng chuỗi đa thức P(x) để hiển thị
    poly_str = ""
...
def main():
    ...
    x1, y1 = pickPoint(x0, num, x, y) # chọn num điểm phù hợp cho Newton lùi
    ...
    print("\n" + "="*80)
    print("KẾT QUẢ NỘI SUY NEWTON LÙI".center(80))
    print("="*80)
    
    x_n = x1[-1]
    print("Các hệ số Newton b_k = ∇^k(y_n) / k!:")
    print(newton_coeffs)
    print(f"\nCông thức dạng Newton: P(t) = b_0 + b_1*t + ... với t = (x - {x_n})/{round(h, 6)}")
    ...
    poly_x_str = get_polynomial_in_x_str(P_t, h, x_n)
    print("\n" + "="*90)
    ...
    plot(x1, y1, x, y, x0, value, P_t, h, x_n)
