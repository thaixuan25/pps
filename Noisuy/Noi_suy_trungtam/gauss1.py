...
def in_bang_tich_gauss(n):
    print("\n" + "="*120)
    print("BẢNG TÍNH TÍCH GAUSS G_k(t)".center(120))
    print("="*120)
    ...
    for k in range(1, n + 1):
        ...
        # Tính p_k tương ứng với công thức Gauss
        if k % 2 == 0: # k chẵn
            p = k // 2
        else: # k lẻ
            p = -(k - 1) // 2
...
# Áp dụng công thức Gauss tiến
def gauss_forward_interpolation(n, x, y, x_interpolate):
    """
    Thực hiện nội suy Gauss tiến (Gauss I).

    Công thức tổng quát:
    P(x) = y₀ + t⋅Δy₀ + t(t-1)/2! ⋅ Δ²y₋₁ + (t+1)t(t-1)/3! ⋅ Δ³y₋₁ + (t+1)t(t-1)(t-2)/4! ⋅ Δ⁴y₋₂ + ...

    Trong đó:
    - t = (x - x₀) / h
    - x₀ là giá trị mốc được chọn (thường ở trung tâm)
    - h là bước nhảy (khoảng cách đều giữa các điểm x)
    - y₀ là giá trị y tại x₀
    - Δy₀, Δ²y₋₁, Δ³y₋₁, ... là các sai phân hữu hạn tương ứng được chọn theo đường ziczac
      từ bảng sai phân, bắt đầu từ y₀.

    Các bước thực hiện thuật toán:
    1. Chuẩn bị dữ liệu: Đọc các điểm (x_i, y_i) và giá trị x cần nội suy. 
       Kiểm tra để đảm bảo các điểm x_i cách đều nhau (h = x_{i+1} - x_i là hằng số).
    2. Chọn điểm gốc x₀: Tìm điểm x_i trong dãy dữ liệu gần nhất với giá trị x cần nội suy.
       Điểm này được chọn làm điểm mốc x₀.
    3. Tính tham số t: Tính giá trị t = (x - x₀) / h. Đây là biến số trong đa thức nội suy.
    4. Lập bảng sai phân: Xây dựng bảng sai phân hữu hạn từ các giá trị y_i.
    5. Áp dụng công thức:
       - Từ bảng sai phân, lấy các giá trị theo đường ziczac: y₀, Δy₀, Δ²y₋₁, Δ³y₋₁, Δ⁴y₋₂, ...
       - Thay các giá trị sai phân này và giá trị t đã tính vào công thức Gauss tiến để 
         tìm đa thức nội suy P(t) và tính giá trị tại điểm cần tìm.
    """
    h = x[1] - x[0]
    ...
    in_bang_tich_gauss(n - 1)

    # Lấy ra các hệ số sai phân được chọn
    selected_diffs = []
    for k in range(n):
        diff_index = x0_index - (k // 2)
    ...
    seq = [-(i // 2) for i in range(1, len(selected_diffs)+1)]
    ...
    for k in range(1, n):
        ...
        # Xây dựng tích các term của t theo công thức Gauss tiến
...
def main():
    try:
        ...
        result = gauss_forward_interpolation(n, x, y, x_interpolate)
